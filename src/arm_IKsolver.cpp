...
static const double DPOS_FAC = 0.001 * LENGTH_FAC;
...
void update_dpos(const tk_arm::TargetFound& msg)
{
    if (msg.object_recognized) {
        updatingpos = true;
        dpos_new.p(1) = (IMAGE_WIDTH / 2 - msg.object_rect_left) * DPOS_FAC;
        dpos_new.p(2) = (IMAGE_HEIGHT / 2 - msg.object_rect_top) * DPOS_FAC;
        updatingpos = false;
...
int main(int argc, char **argv)
{
    ...
    while(ros::ok())
    {
        // pos input
        // get_pos(pos_goal);
        if (pos_goal.p(0) < obj_dep)
            pos_goal.p(0) += FORWARD_VELO * LENGTH_FAC;

        // update_dpos();

        if (updatingpos)
            pos_goal_corrected.p = pos_goal.p + dpos_old.p;
        else
